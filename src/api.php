...
class MySQL extends ConnectionTools
{
    ...
    public function doOperation($operation, $transop)
    {
        ...
        switch ($operation['method']) {
            ...
            case 'login':
                ...
                while ($row = $result->fetch_assoc()) {
                    // $this->conn->query("delete from tokens where token like  '".$row['username']."%'");
                    if ($this->auth) {
        ...
        switch ($operation['method']) {
            case 'get':
                ...
                $limit = isset($operation["limit"]) && !empty($operation["limit"]) ? " LIMIT " . $operation["limit"] : "";
            ...
            case 'put':
                ...
                foreach ($operation['body'] as $key => $value) {
                    $value = trim($value);
                    if (substr($value, 0, 5) == "__add") {
                        $sql = $sql . "`" . $key . "` =" . $key . " + " . trim(substr($value, 5)) . ",";
                    } else {
                        if (substr($value, 0, 5) == "__min") {
                            $sql = $sql . "`" . $key . "` =" . $key . " - " . trim(substr($value, 5)) . ",";
                        } else {
    ...
    private function doQueryret($sql)
    {
        // die($sql);
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
    ...
    private function doAuth()
    {
        ...
        if ($row) {
            ...
            $resobj = json_encode(array('status' => false, 'message' => 'Unauthorized Aceess. Keep Away!!'), JSON_FORCE_OBJECT);
            array_push($this->res, json_decode($resobj));
            return false;
        }
    }
